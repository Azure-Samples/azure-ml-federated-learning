...
class FederatedLearningPipelineFactory:
    ...
    def build_flexible_fl_pipeline_with_separate_preprocessing(
            self,
            silo_preprocessing, 
            silo_subgraph,
            orchestrator_aggregation,
        ...
        def subgraph_component(running_checkpoint: Input(optional=True), iteration, train_data_0: Input()=None, train_data_1: Input()=None, train_data_2: Input()=None, test_data_0: Input()=None, test_data_1: Input()=None, test_data_2: Input()=None):
            # collect all outputs in a dict to be used for aggregation
            silo_training_outputs = []
            # map of preprocessed outputs
            # silo_preprocessed_outputs = {}
            ...
            for silo_index, silo_config in enumerate(self.silos):
                ...
                datastore = silo_config.get("datastore")
                if silo_index==0:
                    silo_subgraph_output = silo_subgraph(train_data_0, test_data_0, running_checkpoint, iteration, compute, datastore, orchestrator_datastore)
                elif silo_index==1:
                    silo_subgraph_output = silo_subgraph(train_data_1, test_data_1, running_checkpoint, iteration, compute, datastore, orchestrator_datastore)
                else:
                    silo_subgraph_output = silo_subgraph(train_data_2, test_data_2, running_checkpoint, iteration, compute, datastore, orchestrator_datastore)
        ...
        def _fl_cross_silo_factory_pipeline():
            ...
            for silo_index, silo_config in enumerate(self.silos):    
                ...
                preprocess_outputs[f'train_data_{silo_index}'] = preprocessing_outputs["train_data"]
                preprocess_outputs[f'test_data_{silo_index}'] = preprocessing_outputs["test_data"]

            for iteration in range(1, iterations + 1):

                # call pipeline for each iteration
                output_iteration = subgraph_component(running_checkpoint, iteration, **preprocess_outputs)

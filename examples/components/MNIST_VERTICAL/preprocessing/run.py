...
def preprocess_data(
    ...
    train_data = pd.read_csv(raw_training_data, index_col=0)
    test_data = pd.read_csv(raw_testing_data, index_col=0)

    print(train_data.head)
    print(train_data.shape)

    if train_data.shape[1] == 1:
        train_data.to_csv(os.path.join(train_data_dir, "train.csv"))
        test_data.to_csv(os.path.join(test_data_dir, "train.csv"))
        return

    logger.debug(f"Segregating labels and features")
    X_train = torch.tensor(train_data.values)
...
def run(args):
    ...
    preprocess_data(
        os.path.join(args.raw_training_data, "train.csv"),
        os.path.join(args.raw_testing_data, "test.csv"),
        args.train_output,
        args.test_output,
        args.metrics_prefix,
    )

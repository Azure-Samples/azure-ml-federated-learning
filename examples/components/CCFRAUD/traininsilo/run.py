...
class CCFraudTrainer:
    ...
    def load_dataset(self, train_data_dir, test_data_dir, model_name):
        ...
        with read_encrypted(train_data_dir + "/fraud_weight.txt") as f:
            self.fraud_weight_ = np.loadtxt(f).item()
        with read_encrypted(train_data_dir + "/data.csv") as f:
            train_df = pd.read_csv(f)
        with read_encrypted(test_data_dir + "/data.csv") as f:

...
## Model Performance
Another important assessing factor for FL is the model performance. Here we also aim at two questions: 1. How does the FL model performance compare to the centralized model trained with only partial data, which is the scenario when FL is not supported and data are confidential and restricted to each region. 2. How does the FL model performance compare to the centralized model trained with data from all silos, which is an ideal situation when all data are eyes-on and could be combined. This is to understand if the distributed-aggregate design of FL has impact on the model performance. For this reason, we tested 3 models: "FL" for model trained with FL in 3 silos, "Centralized-1/3" for 1 model with 1/3 data, and "Centralized-1" for 1 model with all data combined. 

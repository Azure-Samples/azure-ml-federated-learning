...
def init_sampler(rank, world_size, run_id, host, port, ds, shared_dict, batch_size, shuffle=False):
    # import coverage 
    # coverage.process_startup()

    comm = AMLCommSocket(rank, world_size, run_id, host, port)
    sampler = VerticallyDistributedBatchSampler(ds, batch_size, comm, rank, world_size, shuffle=shuffle)
...
class TestVerticallyDistributedBatchSampler(unittest.TestCase):
    def test_vertically_distributed_batch_sampler_basic(self):
        ...
        p1 = mp.Process(target=init_sampler, args=(0, 2, "test_run_id", "localhost", port, dataset1, shared_dict, batch_size))
        p2 = mp.Process(target=init_sampler, args=(1, 2, "test_run_id", "localhost", port, dataset2, shared_dict, batch_size))
        p1.start(); p2.start()
        p1.join(); p2.join()
        
        # Check that the number of batches is correct
        assert shared_dict[0][1] == shared_dict[1][1] == math.ceil(len(data)/batch_size)

        # Check that the batches are distributed correctly
        for i, (batch1, batch2) in enumerate(zip(shared_dict[0][0], shared_dict[1][0])):
            assert torch.all(batch1 == batch2) and torch.all(batch1 == torch.tensor(range(i*batch_size, min((i+1)*batch_size, len(data)))))
    ...
    def test_vertically_distributed_batch_sampler_shuffle(self):
        ...
        p1 = mp.Process(target=init_sampler, args=(0, 2, "test_run_id", "localhost", port, dataset1, shared_dict, batch_size, True))
        p2 = mp.Process(target=init_sampler, args=(1, 2, "test_run_id", "localhost", port, dataset2, shared_dict, batch_size, True))
        p1.start(); p2.start()
        p1.join(); p2.join()

...
tasks = TesApiInstance.list_tasks(view="mini", task_group=task_group_name)
print(tasks)
print("")

# If some tasks were found, let's show the details of the first one
if len(tasks) > 0:
    task_details = TesApiInstance.get_task(tasks[0]['runId'], view='full')
    print(task_details)
    print("")

# Let's create a task! See the create_task() docstring for more info.
created_task_id = TesApiInstance.create_task(
...
created_task_details = TesApiInstance.get_task(created_task_id, view='full')
print(created_task_details)
print("")

# And finally let's cancel it
TesApiInstance.cancel_task(created_task_id)
